...
const SetCitizenRegistry = ({ govAddress }: SetCitizenRegistryProps) => {
  ...
  const handleSetCitizenRegistry = async () => {
    try {
      await writeContract({
        address: govAddress,
        abi: polityGovernmentAbi,
        functionName: "setCitizenRegistry",
        args: [newAddressForCitizenRegistry],
      });
    } catch (error) {
  ...
  return (
    <div className="pt-4">
      ...
      />
      <button
        onClick={handleSetCitizenRegistry}
        disabled={isPending}
