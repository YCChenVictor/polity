...
const SetVotingMechanism = ({ govAddress }: SetVotingMechanismProps) => {
  ...
  const handleSetVotingMechanism = async () => {
    try {
      await writeContract({
        ...
        args: ["Voting", newAddressForVotingMechanism],
      });
    } catch (error) {
  ...
  return (
    <div className="pt-4">
      ...
      >
        {isPending ? "Setting..." : "Set Voting Mechanism"}
      </button>
      {isSuccess && (
        <p className="text-green-500">Voting Mechanism set successfully.</p>
      )}
